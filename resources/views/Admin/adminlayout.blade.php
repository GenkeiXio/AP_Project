...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root { --green:#6dbf7e; --green-dark:#4da862; --orange:#e8922a; --brown:#3d2a1a; --sidebar-w:260px; }
        body { font-family:'Nunito',sans-serif; background:#f0f4ec; color:var(--brown); display:flex; min-height:100vh; }
        .sidebar { width:var(--sidebar-w); background:linear-gradient(180deg,#2d5a1e 0%,#3d7a2a 100%); min-height:100vh; display:flex; flex-direction:column; padding:28px 0; position:fixed; top:0; left:0; box-shadow:4px 0 20px rgba(0,0,0,0.15); z-index:50; }
        .sidebar-brand { padding:0 24px 28px; border-bottom:1px solid rgba(255,255,255,0.12); }
        .sidebar-brand .logo { font-family:'Baloo 2',cursive; font-size:1.4rem; font-weight:800; color:#fff; line-height:1.1; }
        ...
        .nav-link:hover, .nav-link.active { background:rgba(255,255,255,0.1); color:#fff; border-left-color:#a8e6b0; }
        ...
        .modal { background:#fff; border-radius:20px; padding:32px 36px; width:min(460px,92vw); box-shadow:0 20px 60px rgba(0,0,0,0.2); position:relative; animation:modalIn 0.3s cubic-bezier(.22,1,.36,1) both; }
        ...
        .modal h2 { font-family:'Baloo 2',cursive; font-size:1.25rem; font-weight:800; margin-bottom:20px; color:var(--brown); }
        .form-group { margin-bottom:16px; }
        .form-group label { display:block; font-size:0.85rem; font-weight:700; margin-bottom:6px; color:#5a4030; }
        .form-group input, .form-group select { width:100%; padding:11px 14px; border:2px solid #e0d0ba; border-radius:10px; font-family:'Nunito',sans-serif; font-size:0.93rem; outline:none; transition:border-color 0.2s; }
...
<body>
    <aside class="sidebar">
        ...
        <nav>
            ...
            <a href="#" class="nav-link" onclick="openCreateAdminModal(); return false;">
    ...
    <div class="modal-overlay" id="createTeacherModal">
        <div class="modal">
            ...
            <div class="form-group"><label>Full Name</label><input type="text" id="tName" placeholder="Juan dela Cruz" /></div>
            <div class="form-group"><label>Email</label><input type="email" id="tEmail" placeholder="teacher@school.com" /></div>
            <div class="form-group"><label>Password</label><input type="password" id="tPassword" placeholder="Min. 8 characters" /></div>
            ...
            <button class="btn btn-green btn-full" id="createTeacherBtn" onclick="submitCreateTeacher()">✅ Create Teacher Account</button>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="createAdminModal">
        <div class="modal">
            ...
            <div class="form-group"><label>Full Name</label><input type="text" id="aName" placeholder="Maria Santos" /></div>
            <div class="form-group"><label>Email</label><input type="email" id="aEmail" placeholder="admin2@school.com" /></div>
            <div class="form-group"><label>Password</label><input type="password" id="aPassword" placeholder="Min. 8 characters" /></div>
            <button class="btn btn-orange btn-full" id="createAdminBtn" onclick="submitCreateAdmin()">✅ Create Admin Account</button>
        </div>
    </div>
    ...
    <script>
        ...
        async function submitCreateTeacher() {
            const btn=document.getElementById('createTeacherBtn'), alert=document.getElementById('teacherAlert');
            const body={ name:document.getElementById('tName').value.trim(), email:document.getElementById('tEmail').value.trim(), password:document.getElementById('tPassword').value, avatar:document.getElementById('tAvatar').value };
            if(!body.name||!body.email||!body.password){ showAlert(alert,'error','Please fill in all fields.'); return; }
            btn.disabled=true; btn.innerHTML='<span class="btn-spinner"></span> Creating...';
            try {
                const res=await fetch('{{ route("admin.create-teacher") }}',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF},body:JSON.stringify(body)});
                const data=await res.json();
                if(data.success){
                    showAlert(alert,'success',`✅ Teacher created! Access Code: ${data.access_code} — Save this!`);
                    document.getElementById('tName').value=''; document.getElementById('tEmail').value=''; document.getElementById('tPassword').value='';
                    if(typeof window.onTeacherCreated==='function'){ window.onTeacherCreated(data.teacher,data.access_code); closeModal('createTeacherModal'); }
            ...
            btn.disabled=false; btn.innerHTML='✅ Create Teacher Account';
        }

        async function submitCreateAdmin() {
            const btn=document.getElementById('createAdminBtn'), alert=document.getElementById('adminAlert');
            const body={ name:document.getElementById('aName').value.trim(), email:document.getElementById('aEmail').value.trim(), password:document.getElementById('aPassword').value };
            if(!body.name||!body.email||!body.password){ showAlert(alert,'error','Please fill in all fields.'); return; }
            btn.disabled=true; btn.innerHTML='<span class="btn-spinner"></span> Creating...';
            try {
