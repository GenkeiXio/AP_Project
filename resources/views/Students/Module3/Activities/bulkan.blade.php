...
<head>
    ...
    <style>
        ...
        .video-section { display: flex; gap: 10px; margin-bottom: 20px; }
        .video-section iframe { width: 100%; height: 180px; border-radius: 8px; border: 1px solid #333; }

        .btn-ready {
            background: var(--accent); border: none; color: #000;
            font-weight: 900; letter-spacing: 2px; padding: 15px 30px;
            width: 100%; transition: 0.3s;
        }

        /* --- 2. LAYOUT --- */
        #game-layout { display: flex; width: 100vw; height: 100vh; padding: 25px; gap: 25px; }

        #ui-module {
            flex: 1; background: var(--panel); border-radius: 20px;
            border-left: 8px solid var(--accent); padding: 30px;
            display: flex; flex-direction: column; z-index: 100;
        }

        .status-header { font-size: 0.7rem; letter-spacing: 4px; color: var(--accent); font-weight: 900; margin-bottom: 15px; }
        #scenario-text { font-size: 1.1rem; line-height: 1.4; margin-bottom: 20px; min-height: 120px; }

        .choices-container { display: flex; flex-direction: column; gap: 12px; }
        .stone-btn {
            background: #1a1a1a; border: 1px solid #333; border-bottom: 5px solid #000;
            border-radius: 12px; padding: 18px 20px; cursor: pointer; transition: 0.2s;
        }
        .stone-btn:hover { border-color: var(--accent); transform: translateX(5px); }

        /* --- 3. SIMULATION SIDE --- */
        #simulation-module { 
            flex: 1.5; background: var(--sky); border-radius: 20px; 
        ...
        .green-ground { 
            ...
            background: #1b5e20; border-top: 10px solid var(--safe); z-index: 10; 
        }

        #lava-layer { 
            position: absolute; bottom: -2200px; width: 100%; height: 2000px; 
            background: linear-gradient(0deg, #600, #d35400, #ff4500); z-index: 15; 
            box-shadow: 0 -40px 100px var(--lava); 
        }
        
        .stone-ledge { 
            position: absolute; width: 200px; height: 50px; 
            background: #333; border-top: 8px solid #555; 
        ...
        #safe-place {
            ...
            background: #2c3e50; border: 4px solid var(--safe);
            border-bottom: none; border-radius: 50px 50px 0 0;
            z-index: 11; transform: translateX(-50%);
            display: flex; flex-direction: column; align-items: center; justify-content: flex-end;
            padding-bottom: 20px; box-shadow: 0 0 50px rgba(46, 204, 113, 0.3);
        }
        .safe-door {
            width: 80px; height: 110px; background: #1a252f;
            border: 2px solid var(--safe); border-radius: 10px 10px 0 0;
            transition: 1s ease; position: relative; overflow: hidden;
        }
        .safe-door.open { background: var(--safe); box-shadow: 0 0 30px var(--safe); }
        ...
        .gift-item-img { width: 150px; filter: drop-shadow(0 0 20px var(--accent)); margin-bottom: 20px; }

        .game-overlay { position: absolute; inset: 0; background: rgba(0,0,0,0.95); display: none; flex-direction: column; align-items: center; justify-content: center; text-align: center; padding: 50px; z-index: 2000; }
        
        .next-btn {
            background: var(--safe); color: #000; border: none;
            padding: 15px 30px; font-weight: 900; border-radius: 12px;
            text-decoration: none; display: inline-block; transition: 0.3s;
        }
        .next-btn:hover { background: #27ae60; color: #fff; transform: scale(1.05); }
    </style>
</head>
<body>
...
<div id="instruction-modal">
    <div class="modal-content-custom">
        ...
        <button class="btn-ready" onclick="proceedToDashboard()">TAPUSIN ANG PAGSASANAY</button>
    </div>
</div>
...
<div id="game-layout">
    ...
    <div id="simulation-module">
        <div id="mission-deployment-card" style="display: none; position: absolute; inset: 0; z-index: 1000; background: rgba(0,0,0,0.8); display: flex; align-items: center; justify-content: center; backdrop-filter: blur(5px);">
        ...
        <div id="end-overlay" class="game-overlay">
            ...
            <div class="d-flex flex-column gap-3 w-100 align-items-center">
                ...
                <a href="{{ route('flood.activity') }}" id="nextPageBtn" class="next-btn" style="display:none;">👉 SUSUNOD NA ARALIN (PAGBAHA)</a>
            </div>
        </div>
    </div>
</div>
