		...
		<div class="pretest-card" id="pretestCard">
			...
			<div class="last-score-box" id="lastScoreBox">
				...
				<div class="no-attempts-actions" id="noAttemptsActions">
					<a href="{{ route('inner.map2') }}" class="btn-primary" style="min-width: 200px;">
...
<script>
	...
	function updateRetryIndicator() {
		...
		retryIndicator.textContent = `🔁 Natitirang attempts: ${remainingAttempts} / ${maxRetries}`;
	...
	function restartQuiz() {
		...
		fetch("{{ route('student.module2.pretest.check') }}")
			.then(res => res.json())
			.then(data => {
				if (!data.can_retry) {
			...
			.catch(err => {
				...
				alert('May error sa pag-check ng attempts. Subukan muli.');
			});
	}
		...
		fetch("{{ route('student.module2.pretest.check') }}")
			...
			.then(data => {
				...
				if (!data.can_retry) {
					...
					// Show VN dialog only if quiz is not locked
